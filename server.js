...
function executeBotMove(roomCode, game, liveCurrent) {
  ...
  io.to(roomCode).emit('token-moved', {
    ...
    isSafeSpot: moveRes.isSafeSpot,
    isHomeGoal: moveRes.isHomeGoal,
    homeLaneSixUsed: moveRes.homeLaneSixUsed || false,
...
io.on('connection', (socket) => {
  ...
  socket.on('add-bot', () => {
    ...
    io.to(socket.roomCode).emit('token-moved', {
      ...
      isSafeSpot: res.isSafeSpot,
      isHomeGoal: res.isHomeGoal,
      homeLaneSixUsed: res.homeLaneSixUsed || false,
